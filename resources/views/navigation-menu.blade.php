...
<aside x-cloak
    ...
    <div class="flex items-center h-12 px-2 border-b border-gray-200 dark:border-neutral-700/40 shrink-0 transition-colors duration-200"
        ...
        <a x-show="$store.nav.open" href="{{ route('dashboard') }}" class="flex items-center gap-2 overflow-hidden">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 text-neutral-800 dark:text-neutral-200 shrink-0">
                ...
                <circle cx="19" cy="4" r="2"/><path d="M12.5 7.13288L17.7134 5.5293L15.8766 10.5293"/>
            </svg>
            <span class="text-gray-800 dark:text-white text-xs font-semibold leading-tight truncate">{{ config('app.name') }}</span>
        </a>
        <a x-show="!$store.nav.open" href="{{ route('dashboard') }}">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 text-neutral-800 dark:text-neutral-200">
                <path d="M9 22L6 15L3 22H9Z"/>
        ...
        <button @click="$store.nav.toggle()"
                class="text-gray-500 dark:text-neutral-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-neutral-700 p-1.5 rounded-md transition-colors shrink-0">
            <svg x-show="$store.nav.open" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden py-2 space-y-0.5 px-1.5">
        ...
        @foreach($links as $link)
            ...
            <a href="{{ route($link['route']) }}"
               ...
               @mouseenter="if(!$store.nav.open) tooltip = true"
               ...
               class="group relative flex items-center px-2 py-2 rounded-lg text-sm transition-all duration-150 {{ $active ? 'bg-neutral-600 dark:bg-neutral-700 text-white shadow-md' : 'text-gray-600 dark:text-neutral-400 hover:bg-gray-100 dark:hover:bg-neutral-700/60 hover:text-gray-900 dark:hover:text-white' }}">
                ...
                {{-- Icon --}}
                @if($link['icon'] === 'home')
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0h6"/></svg>
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                @elseif($link['icon'] === 'creditcard')
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                @elseif($link['icon'] === 'cube')
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
                @endif
    ...
    {{-- User Section --}}
    <div class="border-t border-gray-200 dark:border-neutral-700/40 p-1.5 shrink-0 space-y-0.5 transition-colors duration-200">
        {{-- Theme Toggle Button --}}
        <button type="button" @click="$store.nav.toggleTheme()"
                x-data="{ tooltip: false }"
                @mouseenter="if(!$store.nav.open) tooltip = true"
                @mouseleave="tooltip = false"
                :class="$store.nav.open ? 'gap-3' : 'justify-center'"
            ...
            <svg x-show="$store.nav.theme === 'dark'" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0 text-amber-500 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
            </svg>
            {{-- Moon Icon (shown when light) --}}
            <svg x-show="$store.nav.theme !== 'dark'" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0 text-neutral-700 dark:text-neutral-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        <form method="POST" action="{{ route('logout') }}" x-data>
            ...
            <button type="submit" @click.prevent="$root.submit()"
                ...
                <span x-show="$store.nav.open" class="text-xs font-medium truncate">{{ __('Cerrar sesión') }}</span>
            </button>
        </form>
    </div>
</aside>
...
<div x-cloak
     ...
     class="lg:hidden fixed inset-0 z-20 bg-black/60"
     style="display:none;">
</div>
...
<aside x-cloak
    ...
    {{-- Mobile Header --}}
    <div class="flex items-center justify-between h-12 px-3 border-b border-gray-200 dark:border-neutral-700/40 transition-colors duration-200">
        <div class="flex items-center gap-2">
            ...
            <span class="text-gray-800 dark:text-white text-xs font-semibold truncate">{{ config('app.name') }}</span>
        </div>
        <button @click="$store.nav.mobileOpen = false" class="text-gray-500 dark:text-neutral-400 hover:text-gray-900 dark:hover:text-white p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-neutral-700 transition-colors">
    ...
    {{-- Mobile User Section --}}
    <div class="border-t border-gray-200 dark:border-neutral-700/40 p-1.5 space-y-0.5 transition-colors duration-200">
        ...
        <a href="{{ route('profile.show') }}" class="flex items-center gap-3 px-2 py-2 rounded-lg text-sm text-gray-600 dark:text-neutral-400 hover:bg-gray-100 dark:hover:bg-neutral-700/60 hover:text-gray-900 dark:hover:text-white transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
        ...
        <form method="POST" action="{{ route('logout') }}" x-data>
            ...
            <button type="submit" @click.prevent="$root.submit()"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
