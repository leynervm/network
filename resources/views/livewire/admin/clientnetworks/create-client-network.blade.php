<div wire:key="create-client-network">
    ...
    <x-dialog-modal wire:model="open" maxWidth="3xl">
        <x-slot name="title">
            ...
            <button wire:click="$set('open', false)"
                class="rounded-lg text-gray-700 p-2.5 dark:text-gray-400 hover:bg-gray-50 focus:bg-gray-50 dark:hover:bg-neutral-700/40 dark:focus:bg-neutral-700/40 hover:text-gray-600 focus:text-gray-600 dark:hover:text-gray-300 dark:focus:text-gray-300 transition-colors ease-in-out duration-150">
            ...
            <form wire:submit.prevent="save" class="relative w-full grid grid-cols-1 gap-2" x-data="network">
                ...
                @if (validarFibra($type))
                    <div wire:key="create-fiber-loading-container"
                        ...
                        wire:target="type, olt_id, spliter_id, boxnav_id, portboxnav_id"
                        ...
                        <div>
                            ...
                            @if (count($olts) > 0)
                                <x-ubigeo-select wire:model="olt_id" wire:key="create-select-olt" :options="$olts" :searchable="false" placeholder="SELECCIONAR OLT..." class="w-full" />
                            @else
                        ...
                        <div>
                            <x-label value="2. Seleccionar Splitter"
                                class="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5" />
                            <x-ubigeo-select wire:model="spliter_id" wire:key="create-select-splitter" :options="$spliters" :searchable="true" placeholder="SELECCIONAR SPLITTER..." class="w-full" />
                        ...
                        <div>
                            <x-label value="3. Seleccionar Caja NAP"
                                class="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5" />
                            <x-ubigeo-select wire:model="boxnav_id" wire:key="create-select-boxnav" :options="$boxnavs" :searchable="false" placeholder="SELECCIONAR CAJA NAP..." class="w-full" />
                        ...
                        <div>
                            <x-label value="4. Seleccionar Puerto"
                                class="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5" />
                            <x-ubigeo-select wire:model="portboxnav_id" wire:key="create-select-portboxnav" :options="$portboxnavs" :searchable="false" placeholder="SELECCIONAR PUERTO..." class="w-full" />
                ...
                @elseif ($type && !validarFibra($type))
                    <div wire:key="create-antena-loading-container"
                        wire:loading.class="opacity-60 pointer-events-none filter blur-[1.5px]"
                        wire:target="type, antena_id" class="w-full transition-all duration-200">
                        <x-label value="Seleccionar Antena (Inalámbrico / Radio)" />
                        <x-ubigeo-select wire:model.defer="antena_id" wire:key="create-select-antena" :options="$antenas" :searchable="false" placeholder="SELECCIONAR ANTENA..." class="w-full block mt-1.5" />
                ...
                <div class="w-full grid grid-cols-1 gap-2 md:grid-cols-2">
                    <div class="w-full">
                        ...
                        <x-ubigeo-select wire:model.defer="ubigeo_id" :options="$ubigeos" />
        ...
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('leafletMap', () => ({
                ...
                destroyMap() {
                    ...
                    let container = document.getElementById('map-create');
                    if (container) {
                ...
                updateMapView() {
                    ...
                    if (!this.marker) {
                        ...
                        if (document.getElementById('map-create')) {
                            document.getElementById('map-create')._leaflet_marker = this.marker;
                        }
                ...
                updateFromInput() {
                    ...
                    if (!isNaN(newLat) && !isNaN(newLng)) {
                        ...
                        if (this.$wire) {
                            this.$wire.set('latitude', this.lat, true);
                            this.$wire.set('longitude', this.lng, true);
                        }
                    }
                },
                locateMe() {
