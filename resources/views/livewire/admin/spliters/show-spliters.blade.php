        ...
        <div x-show="ponIdx === {{ $si }}" x-cloak wire:key="spliter_{{ $item->id }}">
            ...
            <div
                ...
                <div
                    ...
                    <div class="flex items-center gap-1.5 flex-wrap">
                        @if (count($item->boxnavs) < $item->outs)
                            <button @click="$wire.addbooxnav({{ $item->id }}, fiberIdx)" wire:key="addbn{{ $item->id }}"
                ...
                <div class="bg-gray-50 dark:bg-[#0d0f12] p-2 transition-colors duration-200">
                    <div class="text-[8px]  text-gray-400 dark:text-neutral-600 tracking-widest mb-3 uppercase">
                        // HILOS DE FIBRA ──
                    ...
                    <div class="grid grid-cols-[repeat(auto-fill,minmax(72px,1fr))] gap-1">
                        @for ($f = 1; $f <= $item->outs; $f++)
                            @php $hasNap = $item->boxnavs->firstWhere('splitter_port', $f); @endphp
                            <div class="splitter-port rounded-lg"
                                :class="{ 'selected': fiberIdx === {{ $f - 1 }} }"
                                @click="fiberIdx = (fiberIdx === {{ $f - 1 }}) ? null : {{ $f - 1 }}">
                                <span class="port-title-meta">H-{{ str_pad($f, 2, '0', STR_PAD_LEFT) }}</span>
            ...
            <div x-show="fiberIdx !== null" x-cloak>
                ...
                @foreach ($item->boxnavs as $ni => $itemboxnav)
                    <div x-show="fiberIdx === {{ $itemboxnav->splitter_port - 1 }}" x-cloak wire:key="boxnav_{{ $itemboxnav->id }}">
                                    ...
                                    @if (count($itemboxnav->portboxnavs) > 0)
                                        @foreach ($itemboxnav->portboxnavs as $portboxnav)
                                            <div class="relative group"
                                                ...
                                                <div
                                                    class="rj45-jack rounded-lg {{ $portboxnav->network ? 'connected' : '' }}">
                                                    <span class="port-title-meta">{{ $portboxnav->code }}</span>
                                                    <div class="jack-hardware-plate">
                                                        <div class="hardware-jack-cavity">
                                                            <div class="jack-gold-pins"></div>
                                                        </div>
